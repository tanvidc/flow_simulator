function [ error ] = transerrorFxn(dx)
...
[~, shock_index] = max(dfw2);
rr2 = shock_index:N;
% valid saturation range within BL solution
sw2 = [sw(1) sw(1) sw(rr2)];
vw2 = vw(rr2);
t = 10;
vw2 = [L/t vw2(1)+10E-4 vw2]; %can't interpolate if 2y for 1x. So add 10E-4

x2 = vw2*t;
vq=interp1(x2,sw2,x);

error=max(abs(vq(:)-s(:,t)))/N;
